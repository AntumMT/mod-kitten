...
# Script to fetch changes from upstream mobs_animal mod.
...
upstream = "https://codeberg.org/tenplus1/mobs_animal"
ref = "master"
mob_name = "kitten"
mob_title = "Kitten"
...
def updateNamespace(target):
	...
	content = re.sub("\"mobs_animal:", "\"mobs:", content, flags=re.M)
	...
	for idx in reversed(range(len(lines))):
		...
		if li.startswith("mobs:register_egg(") or li.startswith("mobs:alias_mob(\"mobs:{}\"".format(mob_name)) or li == "-- spawn egg" or li == "-- compatibility with older mobs mod":
			lines.pop(idx)
		elif ".get_translator(\"mobs_animal\")" in li:
			lines[idx] = li.replace("\"mobs_animal\"", "\"mobs_{}\"".format(mob_name))
	...
	add_info = [
		...
		"",
		"mobs:alias_mob(\"mobs_animal:{0}\", \"mobs:{0}\") -- compatibility".format(mob_name)
	]
...
def updateTextures():
	...
	if not os.path.exists("textures"):
		os.mkdir("textures")
	textures = ("inv", "black", "ginger", "sandy", "splotchy", "striped")
	for t in textures:
		file_path = "textures/{}_{}.png".format(file_basename, t)
		download(getRefUrl(file_path), file_path, False)
	file_path = "textures/mobs_hairball.png"
...
if __name__ == "__main__":
	...
	# settings options
	download(getRefUrl("settingtypes.txt"), exit_on_error=False)
